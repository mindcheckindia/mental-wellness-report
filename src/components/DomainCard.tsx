...
import { Domain } from '../../api/_lib/types';
import { getStylesForScore } from '../utils/helpers';
import { InformationCircleIcon, LightbulbIcon, UsersIcon } from './icons';
...
const DomainCard: React.FC<DomainCardProps> = ({ domain, index, firstName }) => {
  ...
  return (
    <div id={`domain-${index}`} className={`bg-white rounded-2xl shadow-lg border-l-8 transition-shadow duration-300 hover:shadow-xl ${borderColor}`}>
        <div className="p-6">
            {/* Header */}
            <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-4 sm:gap-x-4">
                 <h3 className="text-2xl font-bold text-blue-800 mb-2 sm:mb-0 text-left">
                    {domain.name}
                 </h3>
                 <p className={`text-xl font-bold ${textColor} text-left sm:text-right`}>
